...
const largest = (aa) => {
    ...
    // Returns the value of the element in a 2d array for the given row and col.
    // Or, returns a default value if the row or column is out of bounds (returns 0).
    const getCell = (aa, row, col) => {
        if (row < 0 || col < 0) {
            return 0
        }
        if (row >= aa.length) {
    ...
    // Helper function
    const updateLargest = (r1, c1, r2, c2) => {
